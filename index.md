...
I am <a href="https://rehan-malak.github.io/">Rehan MALAK</a> and this page talks about formal methods oriented towards system __performance__ and __safety__ : static analysis, proof assistants, domain specific
languages, scheduling, tracing and modeling.

# Introduction : a long time ago

###  2012-2016 : High Performance Computing

From 2012 to 2016, I did a PhD on lattice QCD which is the discretized model of the strong interactions : those implying the stability of
the nuclei of the atoms. Within the Budapest-Marseille-Wuppertal collaboration, we computed challenging contributions to the anomalous
magnetic moment of the muon with a low statistical error and a careful estimation of the systematic error. This has been made possible with
...
(Update 2021 : The BMW collaboration has made other breakthroughs since with this 2021
[paper](https://www.nature.com/articles/s41586-021-03418-1) published in _Nature_

On the architecture side, IBM stopped its BlueGene line and the trend now for HPC centers is to evaluate the advantages of using x86, Power9, ARM, RISC-V with
GPU's and other domain specific accelerators.

### 2017 : Embedded Systems
...
Android has a good documentation, the repo tool (a Python script to manage multiple git repo) is not too hard to master,
...
This page explains some experiences conducted at Inria (Lille, Saclay and Paris) with formal methods oriented towards __performance__ and __safety__. It contains also some links for the curious readers.

# 2018 : the 'S' in "IoT" stands for Security
...
As this was in the context of GitHub Travis-CI, we tried 1. and 3. (QEMU supports a few no-MMU ARM-boards but not the ones on which contiki-NG was ported.)
...
# 2019 (and 2021) : C AST to Why3 AST
...
Update 2021 : I am currently working on the same idea but from unspecified C code. There are several possibilities in the Ocaml landscape
...
# 2020 : Dedukti 3, a proof assistant leveraging higher order rewriting theory
...
On a more abstract side, I helped to adapt from Dedukti2 to Dedukti3, a semi-simplicial model of System F formalized initially by Bruno
...
# 2021 : Linux scheduling : proofs, DSL, performance, liveness

I am currently working with the Inria Whisper team-project. The team is specialized in several aspects of
...
One of the project I am involved in, is to prove a "local work conservation" property on the idle core selection algorithm of the Linux CFS scheduler
...
# 2022 : Safety and Performance, Traces and Models
<!-- ... global warming -->

Two important changes are arriving in the Linux kernel :
- the Rust programming language is finally going to be adopted (at least for the drivers) but this is going to avoid only certain kind of bugs
- the PREEMPT_RT patches (most of them) are finally going to be merged

This has to be understood in a broader context. While the spread of datacenters, 5G, IoT, robotics continues, there are also recurrent
demands of the tech community for :
- safety
- performance
- energy-efficiency
- responsiveness
...
How to reach these - sometimes contradictory - objectives ?

In addition to the current research on scheduling and real-time systems, I am planning to investigate also :
- traces and timing properties, liveness properties (maybe temporal logic)
- compare timing behavior and PREEMPT_RT between embedded x86/ARM/Risc-V boards

<!-- and -->
<!-- - the ecological disaster, the Pacific trash vortex, the holocene extinction, famine, sanitary crisis, the global warming -->
<!-- I don't know if we currently need more technology ? less technology ? -->
<!-- A pragmatic attitude : we don't need to eradicate all the engineers, we just need more better technology. -->
<!-- Problem we don't know what it is. -->

To be continued...

__UPDATE  3/7/2022__

I just moved to Eindhoven (from Paris) and almost simultaneously been informed that my project on the Linux kernel with PREEMPT_RT patches have been selected by the RISC-V Development Board
Seed Program. I will soon receive the latest [StarFive VisionFive development
board](https://rvspace.org/en/Product/VisionFive/Technical_Documents/VisionFive_Single_Board_Computer_Quick_Start_Guide#specifications). This
is going to be interesting !

<center>
<img src="./figs/visionfive.png" alt="drawing" width="100%"/>
</center>
